...
class ConnectionManager:
    ...
    async def broadcast(self, room_id: str, data: _schemas.MessageCreate, db: Session, current_user):
        for connection in self.rooms[room_id]:
            ...
            new_message =_models.Message(user_1_id=user_1_id, user_2_id=user_2_id, owner_id=int(current_user), data=data['msg'])
            db.add(new_message)
            db.commit()
            db.refresh(new_message)
            new_message_pydantic = _schemas.MessageShow(room_id=room_id, msg=new_message.data, created_at=new_message.created_at.strftime("%d/%m/%Y, %H:%M"))
...
def get_old_messages(room_id: str, db: Session = Depends(_database.get_db)):
    user_1_id = int(room_id.split('-')[0])
    user_2_id = int(room_id.split('-')[1])
    query = db.query(_models.Message).where(_models.Message.user_1_id == user_1_id, _models.Message.user_2_id == user_2_id).order_by(_models.Message.created_at).all()
    return query