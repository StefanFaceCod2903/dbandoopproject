...
def create_conversation(conversation: _schemas.ConversationCreate, db: Session = Depends(_database.get_db), current_user: int = Depends(_oauth2.get_current_user)):
    ...
    if (conversation.user_1_id != int(current_user.id) and conversation.user_2_id != int(current_user.id)):
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED,
                            detail="You can only connect yourself with other users.")
    if (conversation.user_1_id > conversation.user_2_id):
        db_conversation = _models.Conversation(
            user_1_id=conversation.user_2_id, user_2_id=conversation.user_1_id, vice_id=conversation.vice_id)
    else:
        db_conversation = _models.Conversation(
            user_1_id=conversation.user_1_id, user_2_id=conversation.user_2_id, vice_id=conversation.vice_id)
...
def show_conversations(db: Session = Depends(_database.get_db), current_user: int = Depends(_oauth2.get_current_user)):
    ...
    query1 = db.query(_models.Conversation).join(u1, u1.id == _models.Conversation.user_1_id).join(u2, u2.id == _models.Conversation.user_2_id).join(
        v, v.id == _models.Conversation.vice_id).where(u1.id == current_user.id).with_entities(u2.id, u2.display_name, u2.created_at, u2.description, v.name)
    query2 = db.query(_models.Conversation).join(u1, u1.id == _models.Conversation.user_1_id).join(u2, u2.id == _models.Conversation.user_2_id).join(
        v, v.id == _models.Conversation.vice_id).where(u2.id == current_user.id).with_entities(u1.id, u1.display_name, u1.created_at, u1.description, v.name)
    ...
    for item in query:
        ...
        conversation_showcase = _schemas.ConversationShowcase(room_id=room_id, user=_schemas.UserOut(
            id=item[0], display_name=item[1], created_at=item[2], description=item[3]), vice_name=item[4])
