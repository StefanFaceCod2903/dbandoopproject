...
class Conversation(Base):
    ...
    user_1_id = Column(Integer, ForeignKey(
        'users.id', ondelete="CASCADE"), primary_key=True)
    user_2_id = Column(Integer, ForeignKey(
        'users.id', ondelete="CASCADE"), primary_key=True)
...
class UserVice(Base):
    ...
    user_id = Column(Integer, ForeignKey(
        'users.id', ondelete="CASCADE"), primary_key=True)
    vice_id = Column(Integer, ForeignKey(
        'vices.id', ondelete="CASCADE"), primary_key=True)
...
class User(Base):
    ...
    connections = relationship("User", secondary="conversations", backref='users', primaryjoin=(
        Conversation.user_1_id == id), secondaryjoin=(Conversation.user_2_id == id),)
